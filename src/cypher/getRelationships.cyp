...
WITH CASE 
	WHEN NOT (from IS NULL OR e1 IS NULL OR e2 IS NULL) 
		THEN {uuid:from.uuid,source:e1.uuid,target:e2.uuid,label:type(from),left:false,right:true} 
	END as from
...
WITH fromLinks, CASE 
	WHEN NOT (to IS NULL OR e1 IS NULL OR e2 IS NULL) 
		THEN {uuid:to.uuid,source:e1.uuid,target:e2.uuid,label:type(to),left:true,right:false} 
	END as to
